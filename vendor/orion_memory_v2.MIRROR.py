...
def _safe_graph_dump(data, filepath):
    """Write the graph the way every other writer does: exclusive lock, private temp, fsync,
    atomic replace.

    The previous `open(filepath, 'w')` truncated the live file to zero before writing ~2 MB, so
    every save left a window in which the graph was EMPTY. On 2026-09-02 at 06:34 the integrity
    watchdog read the file inside that window, correctly refused to repair it, and alerted James.
    Nothing was lost, but the window is the bug."""
    import json as _json
    try:
        import sys as _sys
        _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from orion_brain_portable import _atomic_dump
        _atomic_dump(data, str(filepath), indent=2, do_fsync=True)
        return
    except Exception:
        pass
    try:
        ...
        _fcntl = None
    lockf = None
    tmp = "%s.tmp.%d" % (filepath, os.getpid())
    try:
        if _fcntl:
            lockf = open(str(filepath) + ".lock", "w")
...
class GraphMemory:
    ...
    def save(self, filepath):
        ...
        try:
            _safe_graph_dump(data, filepath)
        except OSError as e:
            try:
                from orion_substrate import publish as _publish
                _publish('brain.storage.degraded', {'path': str(filepath), 'error': f'{type(e).__name__}: {e}', 'errno': getattr(e, 'errno', None)})
            except Exception:
                pass
            raise
...
if __name__ == "__main__":
    ...
    g.store("himalaya is the email tool at /Users/servermac/bin/himalaya", "tool", 1.0, ["email", "send", "himalaya"])
