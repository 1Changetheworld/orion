...
- **If those tools are NOT present** → you are NOT wired in. The Orion brain is unreachable from this Gemini CLI session. Do not pretend otherwise.
...
# then restart Gemini CLI so the new MCP registration loads
```
...
- Each AI CLI has its own MCP registry — Gemini reads `~/.gemini/settings.json` `mcpServers`
